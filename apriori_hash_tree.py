...
def num2Id(datasets, te_array):
    ...
    mapping = {}
    visited = set()
    # print("te_array.indptr[1:]:",te_array.indptr[1:])
    for idx, p_end in enumerate(te_array.indptr[1:]):
        dataset = datasets[idx]
        dataset = sorted(dataset)
        encode = te_array.indices[p_start:p_end]
        encode = sorted(encode, key = int)
        # print("dataset:",dataset)
        # print("encode:",encode)
        for code, data in zip(encode, dataset):
...
def apriori_student(df, dataset, te_array, min_support=0.6):
    """
